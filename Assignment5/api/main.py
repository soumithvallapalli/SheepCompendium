...
def update_one_sandwich(sandwich_id: int, sandwich_data: schemas.SandwichUpdate, db: Session = Depends(get_db)):
    sandwich = sandwiches.read_one(db, sandwich_id=sandwich_id)
    if sandwich is None:
        raise HTTPException(status_code=404, detail="Sandwich not found")
    return sandwiches.update(db=db, sandwich_id=sandwich_id, sandwich=sandwich_data)
...
def update_one_resource(resource_id: int, resource_data: schemas.ResourceUpdate, db: Session = Depends(get_db)):
    resource = resources.read_one(db, resource_id=resource_id)
    if resource is None:
        raise HTTPException(status_code=404, detail="Resource not found")
    return resources.update(db=db, resource_id=resource_id, resource=resource_data)
...
def update_one_recipe(recipe_id: int, recipe_data: schemas.RecipeUpdate, db: Session = Depends(get_db)):
    recipe = recipes.read_one(db, recipe_id=recipe_id)
    if recipe is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return recipes.update(db=db, recipe_id=recipe_id, recipe=recipe_data)
...
def update_one_order_detail(order_detail_id: int, order_detail_data: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    order_detail = order_details.read_one(db, order_detail_id=order_detail_id)
    if order_detail is None:
        raise HTTPException(status_code=404, detail="Order Detail not found")
    return order_details.update(db=db, order_detail_id=order_detail_id, order_detail=order_detail_data)
